...
from Graphics.UI.ui_scenarios import ScenarioScreen
...
class SimulationRenderer:
    def __init__(self, width: int = 1280, height: int = 720, title: str = "Antigravity Raylib Physics Engine"):
        ...
        # Initialize UI sidebar panel and classroom widgets
        self.ui_panel = Panel(15, 60, 270, 415, "Classroom Controls")
        ...
        # Navigation Controls
        self.btn_scenarios = Button(30, 380, 240, 34, "Scenarios Tab (Import/Export)")
    ...
    def handle_input(self) -> None:
        ...
        is_over_panel = pr.check_collision_point_rec(mouse_pos, self.ui_panel.rect)
        ...
        elif pr.is_key_pressed(pr.KeyboardKey.KEY_M):
    ...
    def render_hud(self) -> None:
        ...
        pr.draw_text(f"ANTIGRAVITY CLASSROOM {state_str} | GPU: {mode_str} | RES: {res_str}", 15, 15, 20, state_color)
        pr.draw_text("Controls: [SPACE/P] Play/Pause | [S] Stop | [Left Arrow] Rewind | [M] 2D/3D | [G] Grid | [V] Vectors", 15, 40, 15, pr.GRAY)
        ...
        if self.btn_rewind.update_and_draw():
            self.sim.rewind(steps=60) # Jump back 1 full second
        elif is_rewind_hovered and pr.is_mouse_button_down(pr.MouseButton.MOUSE_BUTTON_LEFT):
            self.sim.rewind(steps=5)  # Continuous smooth rewind while holding

        # Spawning logic (activates placement mode)
        ...
        if self.btn_scenarios.update_and_draw():
            self.scenario_screen.refresh_list()
            self.current_screen = AppScreen.SCENARIOS
    ...
    def render_frame(self) -> None:
        ...
        if self.current_screen == AppScreen.MAIN_MENU:
            ...
            if next_screen != AppScreen.MAIN_MENU:
                if next_screen == AppScreen.QUIT:
                    ...
                    self.current_screen = next_screen
                elif next_screen == AppScreen.SCENARIOS:
                    self.scenario_screen.refresh_list()
                    self.current_screen = next_screen
                else:
                    self.current_screen = next_screen
        ...
        if self.current_screen == AppScreen.SCENARIOS:
            next_screen = self.scenario_screen.update_and_draw()
            if next_screen != AppScreen.SCENARIOS:
                self.current_screen = next_screen
            pr.end_drawing()
            return
        ...
        if self.mode_3d:
            ...
            if self.placement_mode:
                ...
                if abs(ray.direction.y) > 0.0001:
                    ...
                    if t > 0:
                        ...
                        # Check click outside sidebar panel
                        is_over_panel = pr.check_collision_point_rec(mouse_pos, self.ui_panel.rect)
                        if pr.is_mouse_button_pressed(pr.MouseButton.MOUSE_BUTTON_LEFT) and not is_over_panel:
            ...
            for s in self.sim.scene.shapes:
                proj_x = center_x + int(s.pos.x * 25)
                proj_y = center_y - int(s.pos.y * 25)
                pr.draw_circle(proj_x, proj_y, int(s.radius * 25), s.color)
